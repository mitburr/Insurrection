...
class Action:
    ...
    def block_phase(self, players_without_player: FunctionType, target_player: Player = None ) -> tuple[bool,Player]:
        ...
        if self.action_card == Assassin():
            blocking_player = target_player.decision(f"{self.active_player.name} is attempting to assassinate {target_player.name}. Would you like to block your own assassination?", self.game_state ,  {"choice_type": "yes/no", "choices":  ["yes","no"]})
        else:
            blocking_player = next((p for p in players_without_player(self.active_player) if p.decision(f"{p.name} would you like to block?", self.game_state, {"choice_type": "yes/no", "choices":  ["yes","no"]} )), False)
...
class Coup(Action):
    ...
    def coup_action(self, players_without_player):
        ...
        coup_target_name = self.active_player.decision(f"Which player would you like to launch a coup against?", self.game_state, {"choice_type": "player", "choices": [p.name for p in players_without_current_player]})
...
class Assassinate(Action):
    ...
    def assassinate_action(self, players_without_player):
        ...
        players_without_current_player = players_without_player(self.active_player)
        target_name = self.active_player.decision("Who would you like to assassinate?", self.game_state, {"choice_type" : "player", "choices" : [p.name for p in players_without_current_player]})                        
